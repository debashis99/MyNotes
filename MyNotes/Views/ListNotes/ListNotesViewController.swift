...
class ListNotesViewController: UIViewController {
    ...
    static let identifier = "ListNoteViewController"
    ...
    func search(_ query: String) {
        if query.count >= 1 {
            filteredNotes = allNotes.filter { note in
                do {
                    let decryptedText = try CoreDataManager.shared.decrypt(note.text)
                    return decryptedText.lowercased().contains(query.lowercased())
                } catch {
                    // Handle decryption error gracefully
                    print("Decryption error: \(error)")
                    return note.text.lowercased().contains(query.lowercased())
                }
            }
        } else {
            filteredNotes = allNotes
